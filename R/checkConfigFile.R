...
checkConfigFile<-function(config){
  ...
  if(is.empty(config$paths$dir_output)){
    ...
    outputDirCreated<-TRUE ## to double check if folder is created successfully
    outputDirCreated<-tryCatch(

      dir.create(config$paths$dir_output),

      error = function(err){
        runStopCommand(sprintf("Could not create outpur directory at \'%s\'! see below error:\n%s",
  ...
  ## 2
  ##change \ to / in paths
  config$paths$dir_data<-gsub(pattern = '\\\\',replacement = '/' , x = config$paths$dir_data)
  config$paths$dir_output<-gsub(pattern = '\\\\',replacement = '/' , x = config$paths$dir_output)
  config$paths$dir_references<-gsub(pattern = '\\\\',replacement = '/' , x = config$paths$dir_references)

  ## double check to remove / at the end of path e.g '/var/user/user1/' to 'var/user/user1'
  config$paths$dir_data<-sub(pattern = '/$',replacement = '' , x = config$paths$dir_data)
  config$paths$dir_output<-sub(pattern = '/$',replacement = '' , x = config$paths$dir_output)
  config$paths$dir_references<-sub(pattern = '/$',replacement = '' , x = config$paths$dir_references)
  ...
  input.file.names <- list.files(path = config$paths$dir_data,
  ...
  input.file.names <- input.file.names[file_ext(input.file.names) %in% c('gz','zip','txt','dat','csv')]

  ## check if file exists
  if(length(input.file.names) == 0)
  {
    runStopCommand(sprintf("Input file not found at \'%s\'! (['gz|zip|txt|dat|csv'] extensions are accepted)!",config$paths$dir_data))
  }else{
  ...
  found.file.template <- paste(sapply(config$paths$filename ,function(x) tools::file_path_sans_ext(basename(x))) ,
                               collapse = '|')

  existing.files <- list.files(path = config$paths$dir_output,
  ...
  config$new_items$non.empty.output.folder <- ifelse(length(existing.files) > 0 , TRUE , FALSE)
  ...
  if(is.empty(config$supplementaryFiles$allele_ref_std)){
    ...
    config$supplementaryFiles$allele_ref_std <- sprintf('%s/%s',
                                                        config$paths$dir_references,
                                                        config$supplementaryFiles$allele_ref_std)

    ## check if reference file exists
    if(!file.exists(config$supplementaryFiles$allele_ref_std)){
      runStopCommand(sprintf("Reference file not found at %s!",config$supplementaryFiles$allele_ref_std))
    }
    ...
    if(tools::file_ext(config$supplementaryFiles$allele_ref_std) == 'sqlite' &&
      ...
      runStopCommand('RSQLite package is not installed on this computer!\nyou can use the table version of reference file (RData/RDS/txt).')
  }


  config$supplementaryFiles$allele_ref_std_population <- checkConfigParameters(toupper(config$supplementaryFiles$allele_ref_std_population),
                                                                               'list',
                                                                               'EUR' ,
  ...
  if(is.empty(config$supplementaryFiles$allele_ref_alt)){
    ...
    config$supplementaryFiles$allele_ref_alt <- sprintf('%s/%s',
                                                        config$paths$dir_references,
  ...
  if(is.empty(config$supplementaryFiles$beta_ref_std)){
    ...
    if(!file.exists(config$supplementaryFiles$beta_ref_std)){
      runStopCommand(sprintf("Beta (effect) reference file not found at \'%s\'!",config$supplementaryFiles$beta_ref_std))
    }
  }
  ...
  if(is.empty(config$parameters$na.string))
    config$parameters$na.string <-c("NA", "nan", "NaN", ".")
  else
    config$parameters$na.string <- checkConfigParameters(config$parameters$na.string,
                                                         'text',
                                                         c("NA", "nan", "NaN", "."))

  ## 7
  ##set column separator from config file
  if(is.empty(config$parameters$column_separator))
    config$parameters$column_separator<-'auto'
  else
    config$parameters$column_separator<- checkConfigParameters(config$parameters$column_separator,
                                                               'text',
                                                               'auto')

  ##set effect type string  from config
  if(is.empty(config$parameters$effect_type))
    config$parameters$effect_type <- 'BETA'
  else
    config$parameters$effect_type <- checkConfigParameters(toupper(config$parameters$effect_type),
                                                           'list',
                                                           default = 'BETA',
                                                           range = c("BETA", "OR"))
  ...
  config$parameters$effect_type_string<- ifelse(config$parameters$effect_type == 'BETA' ,
                                                'BETA',
                                                'Ln(OR)')
  ...
  config$paths$filename_output_tag <- checkConfigParameters(config$paths$filename_output_tag,
                                                            'text',
                                                            'QC')


  config$plot_specs$plot_title <- checkConfigParameters(config$plot_specs$plot_title,
                                                        'text',
                                                        'none')

  ## 8
  ## Imputed alleles - T or F
  if(is.empty(config$parameters$imputed_T))
    config$parameters$imputed_T <- "TRUE|T|YES|Y"
  else
    config$parameters$imputed_T <- paste(evaluateListsAsStrings('imputed_T',config$parameters$imputed_T),
                                         collapse = '|')

  if(is.empty(config$parameters$imputed_F))
    config$parameters$imputed_F<- "FALSE|F|NO|N"
  else
    config$parameters$imputed_F <- paste(evaluateListsAsStrings('imputed_F',config$parameters$imputed_F),
                                         collapse = '|')

  ##IMP_QUALITY
  config$filters$minimal_impQ_value  <- checkConfigParameters(config$filters$minimal_impQ_value,'numeric', -0.5)
  config$filters$maximal_impQ_value  <- checkConfigParameters(config$filters$maximal_impQ_value,'numeric', 1.5)

  ##HQ Filters ####
  #####

  config$filters$HQfilter_FRQ  <- checkConfigParameters(config$filters$HQfilter_FRQ,'numeric', 0.01)
  config$filters$HQfilter_HWE  <- checkConfigParameters(config$filters$HQfilter_HWE,'numeric',1e-6)
  config$filters$HQfilter_cal  <- checkConfigParameters(config$filters$HQfilter_cal,'numeric',0.95)
  config$filters$HQfilter_imp  <- checkConfigParameters(config$filters$HQfilter_imp,'numeric',0.3)


  # config$parameters$calculate_missing_p

  config$parameters$calculate_missing_p  <- checkConfigParameters(config$parameters$calculate_missing_p,
                                                                  'logical',FALSE)

  #remove chromosomes from QC
  config$remove_chromosomes$remove_X  <- checkConfigParameters(config$remove_chromosomes$remove_X,
                                                               'logical',FALSE)
  config$remove_chromosomes$remove_Y  <- checkConfigParameters(config$remove_chromosomes$remove_Y,
                                                               'logical',FALSE)
  config$remove_chromosomes$remove_M  <- checkConfigParameters(config$remove_chromosomes$remove_M,
                                                               'logical',FALSE)
  config$remove_chromosomes$remove_XY  <- checkConfigParameters(config$remove_chromosomes$remove_XY,
                                                                'logical',FALSE)

  config$filters$threshold_diffEAF  <- checkConfigParameters(config$filters$threshold_diffEAF,'numeric', 0.15)
  ...
  config$plot_specs$plot_cutoff_p  <- checkConfigParameters(config$plot_specs$plot_cutoff_p,'numeric',0.01)


  config$plot_specs$make_plots  <- checkConfigParameters(config$plot_specs$make_plots,'logical',FALSE)



  ##output items
  config$output_parameters$save_final_dataset <- checkConfigParameters(config$output_parameters$save_final_dataset,
                                                                       'logical',
                                                                       FALSE)

  config$output_parameters$gzip_final_dataset <- checkConfigParameters(config$output_parameters$gzip_final_dataset,
                                                                       'logical',
                                                                       FALSE)

  config$output_parameters$html_report <- checkConfigParameters(config$output_parameters$html_report,
                                                                'logical',
                                                                TRUE)


  config$output_parameters$object_file <- checkConfigParameters(config$output_parameters$object_file,
                                                                'logical',
                                                                TRUE)

  # converted to upppercase for consistency
  config$output_parameters$out_header <- checkConfigParameters(toupper(config$output_parameters$out_header),
                                                               'list',
                                                               'STANDARD' ,
                                                               c('GENABEL','GWAMA','PLINK','META','STANDARD','GCTA'))

  #
  config$output_parameters$out_sep <- checkConfigParameters(config$output_parameters$out_sep,
                                                            'list',
                                                            '\t' ,
                                                            c('\t',';','|',','))
  #
  config$output_parameters$out_na <- checkConfigParameters(config$output_parameters$out_na,
                                                           'text',
                                                           'NA')

  #
  config$output_parameters$out_dec <- checkConfigParameters(config$output_parameters$out_dec,
                                                            'list',
                                                            '.' ,
                                                            c('.'))
  ...
  #
  config$plot_specs$graphic_device <- checkConfigParameters(tolower(config$plot_specs$graphic_device),
                                                            'list',
                                                            'png' ,
                                                            c('png','jpeg','tiff'))
  ...
  config$debug$verbose  <- checkConfigParameters(config$debug$verbose,
                                                 'logical',FALSE)

  config$debug$save_pre_modification_file  <- checkConfigParameters(config$debug$save_pre_modification_file,
                                                                    'logical',FALSE)

  config$debug$reduced.AF.plot  <- checkConfigParameters(config$debug$reduced.AF.plot,
                                                         'logical',TRUE)

  config$debug$test_row_count  <- checkConfigParameters(config$debug$test_row_count,
                                                        'numeric',1000)
  ...
  config$paths$effsizePlotPath<-sprintf('%s/Checkgraph_effect-size%s',
                                        config$paths$dir_output,
                                        .QC$img.extension)

  config$paths$precisionPlotPath<-sprintf('%s/Checkgraph_precision%s',
                                          config$paths$dir_output,
                                          .QC$img.extension)
  ...
  config$paths$html.report<-sprintf('%s/%s_%s',
                                    config$paths$dir_output,
                                    config$paths$filename_output_tag,
                                    'Report.html')

  config$paths$txt.report <- sprintf('%s/%s_%s',
                                     config$paths$dir_output,
                                     config$paths$filename_output_tag,
                                     'Report.txt')

  config$paths$xlsx.report <- sprintf('%s/%s_%s',
                                      config$paths$dir_output,
                                      config$paths$filename_output_tag,
                                      'Report.xlsx')

  config$paths$save_pre_modification_file <- sprintf('%s/%s_%s',
                                                     config$paths$dir_output,
                                                     config$paths$filename_output_tag,
                                                     'early_file.txt')
...
{
  output.list<-tryCatch(eval(parse(text = input.string)),
                        warning = function(err){
                          print.and.log(sprintf('error evaluating \'%s\' parameter in config file! should be written as a vector.',input.name),
                                        'fatal')
                          return(NULL)
                        }, error = function(err) {
                          print.and.log(sprintf('error evaluating \'%s\' parameter in config file! should be written as a vector.',input.name),
  ...
  if(is.vector(output.list))
    ...
    print.and.log(sprintf('\'%s\' parameter should be written as a vector in config file!',input.name),
                  'fatal')
...
printConfigVariables <- function(config){
  ...
  print.and.log(sprintf('Alternate header file: \'%s\'',config$supplementaryFiles$header_translations),
                'info')

  print.and.log(sprintf('Allele Frequency Reference file: \'%s\'',config$supplementaryFiles$allele_ref_std),
                'info')

  if(!is.na(config$supplementaryFiles$allele_ref_alt))
    print.and.log(sprintf('Allele Frequency Alternative Reference file: \'%s\'',config$supplementaryFiles$allele_ref_alt),
                  'info')

  if(!is.na(config$supplementaryFiles$beta_ref_std))
    print.and.log(sprintf('Effect Size Reference file: \'%s\'',config$supplementaryFiles$beta_ref_std),
                  'info')
  ...
  if(!.QC$config$graphic.device)
    print.and.log('No graphic devices are available. Plotting will be skipped!','warning')
...
checkConfigParameters <- function(parameter, type , default , range = NULL) {
  ...
  if (type == 'list')
  {
    ...
    if(is.null(parameter) || length(parameter) == 0 || parameter %notin% range)
      output <- default
    else{
      parameter <- gsub('"','',parameter)# remove " from variables
      output <- parameter
