...
create.xlsx.report <- function(config,study.list){
  ...
  report.table <- t(data.table(
    ...
    sapply(study.list, function(x) return(x$tables$variable.summary['Min.', .QC$config$parameters$effect_type_string])),
    sapply(study.list, function(x) return(x$tables$variable.summary['1st Qu.', .QC$config$parameters$effect_type_string])),
    sapply(study.list, function(x) return(x$tables$variable.summary['Median', .QC$config$parameters$effect_type_string])),
    sapply(study.list, function(x) return(x$tables$variable.summary['Mean', .QC$config$parameters$effect_type_string])),
    sapply(study.list, function(x) return(x$tables$variable.summary['3rd Qu.', .QC$config$parameters$effect_type_string])),
    sapply(study.list, function(x) return(x$tables$variable.summary['Max.', .QC$config$parameters$effect_type_string])),
  ...
  row.names(report.table)<- c('File number',
                              ...
                              .QC$config$parameters$effect_type_string,
                              "      Min.",
                              "      1st Qu.",
                              "      Median",
                              "      Mean",
