...
Operating CPU cores:		**`r .QC$cpu.core`**
...
`r report.table <- t(data.table(
       ...
       "<span class='item'>Duplicated variants</span>" = sapply(.QC$qc.study.list, function(x) return(calculatePercent(x$duplicate.count,x$rowcount.step3,pretty=TRUE))),
	   "<span class='mainItem'>Variant count after step 2</span>" = sapply(.QC$qc.study.list, function(x) return(calculatePercent(x$rowcount.step2,
                                                      ...
                                                      decimal.place=3,
                                                      pretty = T))),
	   "<span class='item'>Mismatch variants</span>" = sapply(.QC$qc.study.list, function(x) return(calculatePercent((x$mismatched.rows.std + x$mismatched.rows.alt),
                                           x$rowcount.step2,
                                           decimal.place=3,
                                           pretty=TRUE))),
       ...
       "<span class='mainItem'>Visscher's statistic (HQ variants)</span>" = sapply(.QC$qc.study.list, function(x) return(x$Visschers.stat.HQ)),
        ...
        "<span class='item'>Min.</span>" = sapply(.QC$qc.study.list, function(x) return(x$tables$variable.summary['Min.',  .QC$config$parameters$effect_type_string])),
	   "<span class='item'>1st Qu.</span>" = sapply(.QC$qc.study.list, function(x) return(x$tables$variable.summary['1st Qu.',  .QC$config$parameters$effect_type_string])),
    	   "<span class='item'>Median</span>" = sapply(.QC$qc.study.list, function(x) return(x$tables$variable.summary['Median', .QC$config$parameters$effect_type_string])),
	   	   "<span class='item'>Mean</span>" = sapply(.QC$qc.study.list, function(x) return(x$tables$variable.summary['Mean', .QC$config$parameters$effect_type_string])),
	   "<span class='item'>3rd Qu.</span>" = sapply(.QC$qc.study.list, function(x) return(x$tables$variable.summary['3rd Qu.', .QC$config$parameters$effect_type_string])),
	           "<span class='item'>Max.</span>" = sapply(.QC$qc.study.list, function(x) return(x$tables$variable.summary['Max.', .QC$config$parameters$effect_type_string])),
